...
# def np2tensor(x):
#     return paddle.to_tensor(x).permute(2, 0, 1).float()
def np2tensor(x):
    paddle.disable_static()
    return paddle.to_tensor(x.astype('float32')).transpose([2, 0, 1])
...
def quantize(x, quantize_range):
    return x.clip(quantize_range[0], quantize_range[1])
...
def get_patch(lr, hr, patch_size, scale, augment_patch=False):
    ...
    lr_x, lr_y = random.randrange(0, lr_w - lr_p + 1), random.randrange(0, 
        lr_h - lr_p + 1)
