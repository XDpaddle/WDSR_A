...
class DIV2K(object):
    ...
    def _prepare(self):
        ...
        for phase in (['train', 'valid'] if self.train else ['valid']):
            ...
            lr_list = sorted(glob.glob('{}/DIV2K_{}_LR_bicubic/X{}/*.png'.
                format(self.args.dataset_dir, phase, self.args.scale)))
    ...
    def _is_ready(self):
        try:
            for phase, num_images in ([('train', 800), ('valid', 100)] if
                self.train else [('valid', 100)]):
    ...
    def __len__(self):
        if self.train:
            repeat = (self.args.batch_size * self.args.iterations_per_epoch //
                self.n_pairs)
        else:
            repeat = 1
    ...
    def __getitem__(self, idx):
        ...
        with h5py.File('{}/DIV2K_{}_x{}.h5'.format(self.args.dataset_dir, 
            'train' if self.train else 'valid', self.args.scale), 'r') as h5:
            if self.train:
                lr, hr = get_patch(h5['lr'][idx], h5['hr'][idx], self.args.
                    patch_size, self.args.scale, self.args.augment_patch)
            else:
