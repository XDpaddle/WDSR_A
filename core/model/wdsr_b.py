...
#         self.module2 = nn.Sequential(weight_norm(nn.Conv2D(n_feats, n_feats *expansion_ratio, kernel_size=1)), 
#             nn.ReLU(),
...
#     def forward(self, x):
#         return x + self.module2(x) * self.res_scale


# class WDSR_B(nn.Layer):

#     def __init__(self, args):
#         super(WDSR_B, self).__init__()
#         head = [weight_norm(nn.Conv2D(3, args.n_feats, kernel_size=3,padding=1))]

#         body = [ResBlock(args.n_feats, args.expansion_ratio, args.res_scale,
#             args.low_rank_ratio) for _ in range(args.n_res_blocks)]
